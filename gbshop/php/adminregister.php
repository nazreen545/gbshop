<?php
    ...
    if (isset($_POST['submit'])) {
        if (!(isset($_POST["name"]) || isset($_POST["email"]) || isset($_POST["phone"]) || isset($_POST["passworda"]) || isset($_POST["passwordb"]))) {
            ...
            echo "<script> window.location.replace('../php/adminregister.php')</script>";
        } else {
            if (file_exists($_FILES["fileToUpload"]["tmp_name"]) || is_uploaded_file($_FILES["fileToUpload"]["tmp_name"])) {
                ...
                $shapass = sha1($passa);
                $otp = rand(1000, 9999);
                $sqlregister = "INSERT INTO tbl_admin(email,phone,name,password,otp) VALUES('$email','$phone','$name','$shapass','$otp')";
                try {
...
<body>
	...
	<div class="main">
		...
		<div class="container">
			<form name="registerForm" action="../php/adminregister.php" onsubmit="return validateRegForm()" method="post" enctype="multipart/form-data">
